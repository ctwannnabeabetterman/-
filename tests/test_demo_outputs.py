...
class DemoSettingsTests(unittest.TestCase):
    ...
    def test_formal_mode_uses_default_waveform_and_one_thousand_trials(self) -> None:
        ...
        self.assertEqual(settings.waveform.sample_rate_hz, 200e6)
        self.assertEqual(settings.waveform.transmit_sample_rate_hz, 400e6)
        self.assertEqual(settings.waveform.carrier_frequency_hz, 5.8e9)
        self.assertEqual(settings.waveform.tone_separation_hz, 40e6)
        self.assertEqual(settings.waveform.pulse_duration_s, 10e-6)
        self.assertEqual(settings.waveform.rise_fall_s, 50e-9)
        self.assertEqual(settings.frequency.rf_carrier_frequency_hz, 4.3e9)
        self.assertEqual(settings.frequency.rf_tone_separation_hz, 10e6)
        self.assertEqual(settings.frequency.reference_frequency_hz, 10e6)
        self.assertEqual(settings.two_way.processing_delay_s, 49.99e-3)
