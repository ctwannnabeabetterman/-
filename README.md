...
对称的 ±20 MHz 复包络相加后，时域实部代数上等于一条 20 MHz 余弦，但上变频后对应 5.780 GHz 与 5.820 GHz 两个 RF 音点。图 01 因而额外把同一复包络映射到 80/120 MHz 等效 IF，以可视方式显示 40 MHz 间隔的拍频；图 02 直接对通信链的复 IQ 做 FFT，显示 ±20 MHz 两个谱峰。等效 IF 只用于绘图，不进入时延估计。
